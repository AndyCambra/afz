...
const CotizadorAlquilerCliente = ({handleChange, handleSubmitThanks, rentUser, handleCancel, errors}) => {
  return (
    ...
    <div className='input-box'>
      <input name="userName" className="rent-user-input" value={rentUser.userName} type="onlyletters" onChange={e => handleChange(e, 'userName', 'onlyletters', true, 75, 3)} placeholder="Nombre" required={true}/>
      {errors['userName'] && <p className='error-white'>{errors['userName']}</p>}
    </div>
    <div className='input-box'>
      <input name="userLastName" value={rentUser.userLastName} className='rent-user-input' type="onlyletters" onChange={e => handleChange(e, 'userLastName', 'onlyletters', true, 75, 3)} placeholder='Apellido' required={true}/> 
